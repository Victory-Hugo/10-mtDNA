...
BASEDIR="/home/luolintao/P1-LLT-23K-mtDNA/10-单倍群分型软件/3-phyloMT"
...
python3 "${PHYLOMT}" \
    ...
    --output "${BASEDIR}/output/新版/fasta/example_fasta_rcrs_output.tsv" \
    --threads ${THREADS} --hits ${HITS} --extended-report

# ── 示例 2：FASTA 输入 × 新版 RSRS 树 ──────────────────────────────────────
python3 "${PHYLOMT}" \
    --tree phylotree-new-rsrs@17.2 \
    --input "${BASEDIR}/input/fasta/example.fasta" \
    --output "${BASEDIR}/output/新版/fasta/example_fasta_rsrs_output.tsv" \
    --threads ${THREADS} --hits ${HITS} --extended-report
...
python3 "${PHYLOMT}" \
    ...
    --output "${BASEDIR}/output/新版/hsd/example_hsd_rcrs_output.tsv" \
    --threads ${THREADS} --hits ${HITS} --extended-report
...
python3 "${PHYLOMT}" \
    ...
    --output "${BASEDIR}/output/新版/vcf/example_wgs_rcrs_output.tsv" \
    --threads ${THREADS} --hits ${HITS} --extended-report
...
python3 "${PHYLOMT}" \
    ...
    --output "${BASEDIR}/output/新版/vcf/example_microarray_rcrs_output.tsv" \
