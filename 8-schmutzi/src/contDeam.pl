...
"\t--noendo\t\t\t\tDo not require deamination on one end to measure the other (Default: off)\n\n".
...
usage() if ( @ARGV < 1 or
	     ! GetOptions('help|?' => \$help, 'split=s' => \$splitPos,'library=s' => \$library,'ref=s' => \$referenceFasta,'title=s' => \$textGraph,'cont=f' => \$contPriorKnow, 'out=s' => \$outputPrefix,'mock' => \$mock, 'uselength' => \$useLength,'lengthDeam=i' => \$lengthDeam,'splitDeam' => \$splitDeam,'noendo' => \$noEndo )
          or defined $help );
...
my $cmdBam2Prof = $bam2prof." -length  $lengthDeam ";
if($noEndo == 0){
  $cmdBam2Prof .= "-endo ";
}
$cmdBam2Prof .= "-".$library." -5p ".$outputPrefix.".endo.5p.prof  -3p ".$outputPrefix.".endo.3p.prof $inbam";
