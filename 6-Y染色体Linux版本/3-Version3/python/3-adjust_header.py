...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='根据样本原始单倍群的首字母，重新调整等级表',
        formatter_class=argparse.RawDescriptionHelpFormatter
    )
    parser.add_argument('--id-hap-file', '-i', required=True,
                        help='ID_Hap.txt 文件路径（第一列为ID，第二列为单倍群）')
    parser.add_argument('--forward-level-file', '-f', required=True,
                        help='正序等级-补齐至下游.txt 文件路径')
    parser.add_argument('--output-file', '-o', required=True,
                        help='输出文件路径')
    
    args = parser.parse_args()
    id_hap_file = Path(args.id_hap_file)
    forward_level_file = Path(args.forward_level_file)
