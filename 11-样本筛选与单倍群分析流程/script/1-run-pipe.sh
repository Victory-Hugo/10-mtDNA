...
# -*- coding: utf-8 -*-
# 样本筛选与单倍群分析流程主管道
# 使用方式：直接编辑下方参数，然后运行此脚本
#
# 示例：
#   1. 编辑 BASE_DIR 和 FORCE_RUN 参数
#   2. 运行：./1-run-pipe.sh
...
# ============================= 用户配置区外部化 ============================= #
# 现在所有用户参数均存放于 conf
CONFIG_FILE="$(dirname "$0")/../conf/1-run.conf"
...
# 处理INPUT_EXCEL路径（支持绝对路径和相对路径）
if [[ "$INPUT_EXCEL" != /* ]]; then
    # 相对路径，基于BASE_DIR
    INPUT_EXCEL="${BASE_DIR}/${INPUT_EXCEL}"
fi
...
# PCA输出目录
PCA_OUTPUT_DIR_GLOBAL="${OUTPUT_DIR}/pca_results_global"
PCA_OUTPUT_DIR_CHINA="${OUTPUT_DIR}/pca_results_china"

# 热图输出目录
HEATMAP_OUTPUT_DIR="${BASE_DIR}/output/频率热图"
...
run_step_2() {
    ...
    local MAPPING_EXCEL="${CONF_DIR}/基础信息速查表.xlsx"
...
run_step_3() {
    ...
    local HAPLOGROUP_CSV="${DATA_DIR}/全部单倍群整理.csv"
    local SAMPLE_CSV="${OUTPUT_DIR}/本次研究样本基本信息.csv"
    local OUTPUT_CSV="${DATA_DIR}/本次研究样本基本信息_含单倍群.csv"
...
run_step_4() {
    ...
    local INPUT_CSV="${DATA_DIR}/本次研究样本基本信息_含单倍群.csv"
    ...
    local PROVINCE_MAPPING="${CONF_DIR}/基础信息速查表.xlsx"
...
run_step_9() {
    ...
    local OUTPUT_PDF_GLOBAL="${BASE_DIR}/output/频率柱状图/Haplogroup_distribution.pdf"

    # 中国输入/输出
    local INPUT_CSV_CHINA="${OUTPUT_DIR}/频率矩阵_中国.csv"
    local PREPARED_CSV_CHINA="${OUTPUT_DIR}/频率分析准备_中国.csv"
    local OUTPUT_PDF_CHINA="${BASE_DIR}/output/频率柱状图/Haplogroup_distribution_中国.pdf"
...
run_step_10() {
    ...
    local R_SCRIPT="${SCRIPT_DIR}/pca_visualization.R"
    local PCA_OUTPUT_DIR="${BASE_DIR}/output/PCA"
...
run_step_11() {
    ...
    local R_SCRIPT="${SCRIPT_DIR}/heatmap_visualization.R"
    local HEATMAP_OUTPUT_DIR="${BASE_DIR}/output/频率热图"
    ...
    local GROUP_MAPPING_GLOBAL="${BASE_DIR}/output/频率热图/Classification_Group_mapping.csv"
    ...
    local GROUP_MAPPING_CHINA="${BASE_DIR}/output/频率热图/Classification_Group_mapping_China.csv"
